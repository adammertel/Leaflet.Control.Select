...
L.Control.Select = L.Control.extend({
  ...
  _setState: function _setState(newState) {
    // events
    if (this.options.onSelect && newState.selected && newState.selected !== this.state.selected) {
  ...
  _isSelected: function _isSelected(item) {
    ...
    if (sel) {
      return this._isGroup(item) ? 'children' in item && item.children.includes(sel) : sel === item.value;
    } else {
      return false;
    }
  },
